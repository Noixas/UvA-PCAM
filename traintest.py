...
if config['load_model'] is None:
    if config['save_model'] is None:
        load_ckpt_path = os.path.join('models',
                                      f'{model.__class__.__name__}_lr{str(optimizer.defaults["lr"]).split(".")[1]}_epoch{config["epochs"]}',
...
for i in range(config['test_runs']):
    _, test_loss, test_acc, test_auc = test(model, test_loader, loss_fun, config['classes'], device,
                                            load_ckpt_path=load_ckpt_path, dropout=dropout)